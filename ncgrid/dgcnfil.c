 ...
 */
    ...
    if ( filnms == NULL ) {
        *iret = -73;
        printf ( "<<<<< Return from dgc_nfil iret = %d\n", *iret );
	return;
    }
 ...
 */
		...
		*iret = -62;
		...
		cmm_free2d( (void **)filnms, &ier );
		*iret = -30;
                printf ( "<<<<< Return from dgc_nfil A iret = %d\n", *iret );
		return;
	    }
		...
		gd_open ( _nfile.crtfnm[im1], &_nfile.outflg[im1], &zero,
		    ...
		    strlen(_nfile.crtfnm[im1]) );
                ...
                if ( ier != 0 ) {
                    cmm_free2d( (void **)filnms, &ier );
                    *iret = -30;
                    printf ( "<<<<< Return from dgc_nfil B iret = %d\n", *iret );
                    return;
                }
                _dgfile.tmpflg[im1] = G_FALSE;
                _dgfile.templt[im1][0] = '\0';
