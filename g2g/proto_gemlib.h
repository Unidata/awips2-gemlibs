 ...
 * J. Wu/CWS  		03/10   Add g2g_* for graph-to-grid in cave	*
 ***********************************************************************/
...
/* DB library */
void db_checkConnection ( char *dbHost, int *iret );
void db_getBinaryData   ( char *dbHost, char *fileName, int *iret );
...
void db_gsatimg       ( char *dbHost, char *queryType, char *source,
                        char *physEl, char *sectorID, char *resolution, char *dbTime,
                        int *iret );
void db_runQuery      ( char *dbHost, char *queryText, char *queryResult,
                        int *iret );
...
void g2g_init( int *iret );
void g2g_param( char *catmap, char *discrete, char *dlines, 
     	 	char *gglimt, char *edgeopts, int *iret, 
		size_t, size_t, size_t, size_t, size_t );
void g2g_calc ( float *grid, int *kx, int *ky, float *hist, int *hstgrd, 
		int *iret);
void g2g_setdata( int *mlines, int *mlatlons, float *mlatPts, float *mlonPts,
                  int *msmthpts, float *msmthLat, float *msmthLon,
                  int * mextpts,  float *mextLat, float *mextLon,
     		  float *mvaluein, int *msmth,  int *mclosed, 
                  int *mmnum, float *mmlat, float *mmlon,
                  float *mmfi, float *mmfj, float *mmvalue,
		  int *iret );
void g2g_edgeopt( int *kx, int *ky, int *iret );
void g2g_findint ( float *grid, float *hist, int *kx, int *ky, int *iret );
void g2g_save ( float *grid, float *hist, int *hstgrd, 
		char *gdfile, char *proj, char *cpyfil, char *gdarea,
                char *anlyss, char *kxky, char *maxgrd, char *gparm,
                char *gdatim, char *gvcord, char *glevel, int *iret,
     		size_t, size_t, size_t, size_t, size_t, size_t,
		size_t, size_t, size_t, size_t, size_t );
int g2g_writer ( float *grid, float *hist, char *hstgrd, 
		char *gdfile, char *proj, char *cpyfil, char *gdarea,
                char *anlyss, char *kxky, char *maxgrd, char *gparm,
                char *gdatim, char *gvcord, char *glevel );
